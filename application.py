...
<html lang=\"en\">
  <head>
    ...
    <style>
      ...
      .card { max-width: 720px; padding: 1.25rem; border: 1px solid #ddd; border-radius: 8px; }
  ...
  <body>
    <div class=\"card\">
      <h2>Fake News Detector Demo</h2>
      <p>Artifacts loaded: {% if model_loaded %}<span class=\"ok\">yes</span>{% else %}<span class=\"warn\">no</span>{% endif %}</p>
      <p><small>Model: <code>{{ model_path }}</code></small></p>
      <form method=\"post\" action=\"/predict-form\">
        <label for=\"message\">Text</label>
        <input type=\"text\" id=\"message\" name=\"message\" placeholder=\"Enter a headline or sentence...\" />
        <div class=\"row\">
          <button type=\"submit\">Predict</button>
        </div>
      </form>
      {% if prediction %}
        <div class=\"row\"><strong>Prediction:</strong> {{ prediction }}</div>
      {% endif %}
      {% if error %}
        <div class=\"row warn\"><strong>Error:</strong> {{ error }}</div>
      {% endif %}
      <div class=\"row\"><small>API: POST <code>/predict</code> with JSON {\"text\": \"...\"}</small></div>
    </div>
  </body>
</html>
"""
...
# Routes
# Root path: keep a simple message consistent with application.py for labs
@application.get("/")
def index():
...
def predict_json():
    ...
    # Accept both keys; prefer 'text' for backwards compatibility with test_api.py
    message = str(data.get("text") or data.get("message") or "").strip()
    if not message:
        return jsonify({"error": "Missing 'text'"}), 400
    try:
        label = _predict_text(message)
        # application.py returns key 'prediction'; mirror that for compatibility
